...
class TestChurnAnalyzer:
    def test_basic_ratio(self):
        ...
        trace = _make_trace(["a.py", "b.py", "a.py"], EventKind.FILE_READ)
        trace.events.append(
            TraceEvent(kind=EventKind.FILE_WRITE, timestamp=_T0, target="a.py", sequence=3)
        )
    ...
    def test_all_writes_is_critical(self):
        analyzer = ChurnAnalyzer()
        report = AnalysisReport(trace_id="t")
        trace = _make_trace(["a.py", "b.py", "c.py"], EventKind.FILE_WRITE)
        analyzer.analyze(trace, _empty_smap(), report)
        m = report.by_name(MetricNames.CHURN_RATIO)[0]
...
class TestLocalityAnalyzer:
    def test_window_hits(self):
        ...
        report = AnalysisReport(trace_id="t")
        smap = _make_smap(["main.py", "utils.py", "main.py", "main.py"])
        analyzer.analyze(Trace(trace_id="t", source="", format=TraceFormat.GENERIC_JSON), smap, report)
    ...
    def test_empty_smap_returns_ok(self):
        analyzer = LocalityAnalyzer()
        report = AnalysisReport(trace_id="t")
        analyzer.analyze(Trace(trace_id="t", source="", format=TraceFormat.GENERIC_JSON), _empty_smap(), report)
        m = report.by_name(MetricNames.LOCALITY_SCORE)[0]
        assert m.value == 1.0
...
class TestThrashingAnalyzer:
    def test_detects_abab_oscillation(self):
        ...
        report = AnalysisReport(trace_id="t")
        # Create clear A→B→A→B pattern across 6 events
        trace = _make_trace(["a.py", "b.py", "a.py", "b.py", "a.py", "b.py"])
        analyzer.analyze(trace, _empty_smap(), report)
...
class TestAnomalyAnalyzer:
    def test_oscillation_detected(self):
        ...
        analyzer.analyze(trace, _empty_smap(), report)
        assert any(a["type"] == "oscillation" for a in report.anomalies)
    ...
    def test_severity_never_crashes(self):
        """High oscillation count must not raise AttributeError (was Severity.WARNING)."""
        analyzer = AnomalyAnalyzer()
        report = AnalysisReport(trace_id="t")
        # 50 oscillations: a→b repeated → score will be > 10
...
class TestContextEntropyAnalyzer:
    def test_zero_entropy_single_pair(self):
        """All transitions are the same pair → zero relative entropy → OK."""
        analyzer = ContextEntropyAnalyzer()
        report = AnalysisReport(trace_id="t")
        trace = _make_trace(["a.py", "b.py", "a.py", "b.py", "a.py", "b.py"])
        analyzer.analyze(trace, _empty_smap(), report)
        m = report.by_name(MetricNames.CONTEXT_ENTROPY)[0]
        assert m.value == pytest.approx(0.0, abs=1e-4)
        assert m.severity == Severity.OK

    def test_high_entropy_many_pairs(self):
        """Many distinct transitions → relative entropy near 1.0 → HIGH or CRITICAL."""
        analyzer = ContextEntropyAnalyzer()
        report = AnalysisReport(trace_id="t")
        # 8 unique transitions, each appearing once → max entropy
...
class TestSemanticContinuityAnalyzer:
    def test_coherent_transitions_score_one(self):
        """All transitions stay in the same directory → fully coherent → OK."""
        analyzer = SemanticContinuityAnalyzer()
        report = AnalysisReport(trace_id="t")
        smap = SemanticMap(trace_id="t")
        # Files in same directory: add_import makes them adjacent
        smap.add_import("src/a.py", "src/b.py")
        smap.transitions = [("src/a.py", "src/b.py"), ("src/b.py", "src/a.py")]
...
class TestDependencyJumpAnalyzer:
    def test_all_distant_jumps_score_one(self):
        """Transitions between files with no import relationship → ratio 1.0 → CRITICAL."""
        analyzer = DependencyJumpAnalyzer()
        report = AnalysisReport(trace_id="t")
        smap = SemanticMap(trace_id="t")
        # Two unrelated files, no imports registered
...
class TestTransitionGraphAnalyzer:
    def test_single_dominant_pair_is_ok(self):
        """One pair dominates (ratio 1.0) → OK (focused workflow)."""
    ...
    def test_uniform_scatter_is_critical(self):
        """All unique transitions (ratio near 0) → CRITICAL (random jumping)."""
        analyzer = TransitionGraphAnalyzer()
        report = AnalysisReport(trace_id="t")
        # 10 unique files → 9 unique transitions, each appearing once → dominant_ratio ≈ 0.11
        trace = _make_trace([f"file_{i}.py" for i in range(10)])
        analyzer.analyze(trace, _empty_smap(), report)
        m = report.by_name(MetricNames.TRANSITION_CONCENTRATION)[0]
        # With 9 unique transitions each count=1, dominant_ratio = 1/9 ≈ 0.111
        assert m.value == pytest.approx(1 / 9, abs=1e-4)
        # 0.111 falls in MEDIUM range (>= 0.10)
